...
rl.question("Ок, перед тем, как мы начнем, введите свой access_token: ", access_token => {
	...
	let files = fs.readdirSync(__dirname).filter(filename => /^comments\d*\.html$/.test(filename))

	if (files.length === 0) {
		console.log("Ошибка: Не найдено ни одного файла с комментариями. Убедитесь, что вы расположили файлы проекта в директорию comments.")
		rl.close()
		return
	}
